...
[Problem Statement] Therefore, we are interested in investigating the relationship between the popularity of TV shows and the number of its production companies: do well-known TV shows are always produced by multiple production companies and why.
...
Firstly, we wanted to analyze ratings among popular TV shows (those with at least 100,000 votes on IMDB). After a first glance, we noticed an interesting pattern that all of the TV shows that are shown in our table have multiple rows with the same title, meaning that they have multiple production companies. Since our table is too large to look through all of it, we created another table that counts the number of production companies of each of the TV shows.
...
To confirm our guess, we also created a table that counts the number of TV shows that are less popular (have less than 100 votes on IMDb) and the number of their production companies.
